...
nu = 10; % à modifier de pour nu > 0
zeta = 1.99; % à modifier pour 0 > zeta > 2
% lambdas = linspace(0.1,1,10);
% stdRange = linspace(15,25,3);
lambdas = [ 0.1 0.2 0.3 0.4 ];%linspace(0.2,0.6,3);
stdRange = 15;
N = length(lambdas);
opt.bound = 'per';
...
for std = stdRange
    jj = 1;
    for lambda = lambdas
        fprintf('Sigma = %g, \t lambda = %.2f\n',std ,lambda);
        fprintf('PSNR \t SSIM\n');
        for im =1:3%nbr_img
            I = double(imread(imgs{im}));
            I = crop(I,128); % A remettre au 128
            % -- Les indices des Cliques ---
            [indD, indC] = generate_indices(I,sqrt(k));
            %
            ...
            imwrite(uint8(f),['./results/image_',int2str(im),'_std_',int2str(std),'Deniosed','.tiff'],'tiff');
            imwrite(uint8(g),['./results/image_',int2str(im),'Nnoised','.tiff'],'tiff');
            % -- Show Results !
            fprintf('%.3f \t %.3f\n',psnr_i(im),ssimval(im));
        end
        % -- PSNR
    ...
    save(['./results\results_std_',int2str(std),'.mat'], 'psnrs', 'psnrs_mean', 'lambda_op','lambdas', 'ssim_value','index');
    
    fprintf('Pour le niveau de bruit Sigma = %g, Lambda Optimale est : %.2f\n', std, lambda_op(kk));
